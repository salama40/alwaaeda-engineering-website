...
.feature-section {
  ...
  .feature-section__grid {
    ...
    @media (min-width: 768px) {
      width: 1194px;
      max-width: 100%;
      grid: auto / repeat(6, 1fr); /* شبكة 6 أعمدة */
      margin: 80px auto 0;
    }
  }
  ...
  .feature-card {
    ...
    background: #141414;
    min-height: 350px; /* هذا هو الارتفاع الافتراضي للكارد */
    display: flex;
    ...
    /* **التعديل الجديد: تأكيد إزالة transform هنا** */
    ...
    /*
     * الكارد الأول يمتد على 6 أعمدة (عرض كامل)
     */
    &:nth-child(1) {
      grid-column: span 1; /* على الشاشات الصغيرة */

      @media (min-width: 768px) {
        grid-column: span 6; /* يمتد على 6 أعمدة بالكامل على الشاشات الكبيرة */
      }
    }

    /*
     * الكاردين الثاني والثالث يمتدان على 3 أعمدة (نصف عرض)
     * مع تقليل الارتفاع (min-height)
     */
    &:nth-child(2),
    &:nth-child(3) {
      grid-column: span 1;
      ...
      @media (min-width: 768px) {
        grid-column: span 3; /* يأخذ 3 أعمدة (نصف الصف) */
        min-height: 200px; /* **تقليل الطول هنا** */
      }
    }
  }

  /*
   * تنسيق خاص لتوزيع البطاقات عندما يكون القسم معكوسًا (feature-section--flip).
   */
